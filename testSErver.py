...
def c_run():
    ...
    for i in tqdm.tqdm(range(test_count)):
        data = {"method": "echo", "message": data, "timestamp": "{time.time()}"}
    ...
    print(
        f"Error {error_count} | Mean time elapsed: {sum(time_elapsed)/len(time_elapsed)}"
    )
...
Server_thread = Thread(target=s_run, args=(myServ,))
